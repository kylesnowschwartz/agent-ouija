...
// pat.SystemOutputTags entries exclude a user message from being a "user chunk" starter.
...
func Classify(e Entry) (ClassifiedMsg, bool) {
	...
	if e.Type == "user" {
		...
		if pat.TeammateMessageRe.MatchString(trimmed) {
			...
			if pat.TeammateProtocolRe.MatchString(innerContent) {
				return nil, false
			}
	...
	if e.Type == "user" {
		trimmed := strings.TrimSpace(contentStr)
		if strings.HasPrefix(trimmed, pat.LocalCommandStdoutTag) || strings.HasPrefix(trimmed, pat.LocalCommandStderrTag) {
			return SystemMsg{
				Timestamp: ts,
		...
		if strings.HasPrefix(trimmed, pat.BashStdoutTag) || strings.HasPrefix(trimmed, pat.BashStderrTag) {
			stderrContent := ""
			if m := pat.BashStderrRe.FindStringSubmatch(contentStr); m != nil {
		...
		if strings.HasPrefix(trimmed, pat.TaskNotificationTag) {
			status := ""
			if m := pat.TaskNotifyStatusRe.FindStringSubmatch(contentStr); m != nil {
...
func extractTeammateID(s string) string {
	m := pat.TeammateIDRe.FindStringSubmatch(s)
	if m == nil {
		return ""
	}
	return m[1]
...
func extractTeammateColor(s string) string {
	m := pat.TeammateColorRe.FindStringSubmatch(s)
	if m == nil {
		return ""
	}
	return m[1]
...
func extractTeammateContent(s string) string {
	m := pat.TeammateContentRe.FindStringSubmatch(s)
	if m == nil {
...
func extractAssistantDetails(raw json.RawMessage) (int, []ToolCall, []ContentBlock) {
	var blocks []pat.ContentBlockJSON
	if err := json.Unmarshal(raw, &blocks); err != nil {
		return 0, nil, nil
	}
