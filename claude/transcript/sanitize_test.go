...
func TestSanitizeContent_CommandOutput(t *testing.T) {
	...
	got := transcript.SanitizeContent(input)
	want := "file1.go\nfile2.go"
	if got != want {
...
func TestSanitizeContent_NoiseTagsStripped(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := transcript.SanitizeContent(tt.input)
			if got != tt.want {
...
func TestExtractText_JSONString(t *testing.T) {
	...
	got := transcript.ExtractText(raw)
	if got != "Hello, world!" {
...
func TestExtractText_EmptyContent(t *testing.T) {
	got := transcript.ExtractText(nil)
	if got != "" {
		t.Errorf("ExtractText(nil) = %q, want empty", got)
	}

	got = transcript.ExtractText(json.RawMessage{})
	if got != "" {
...
func TestIsCommandOutput(t *testing.T) {
	...
	for _, tt := range tests {
		got := transcript.IsCommandOutput(tt.input)
		if got != tt.want {
