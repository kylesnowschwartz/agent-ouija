...
type DisplayItem struct {
	...
	// Tool categorization
	ToolCategory tools.ToolCategory // broad functional group (Read, Edit, Bash, etc.)
			...
			for _, b := range m.Blocks {
				...
				case "tool_use":
					...
					if b.ToolName == "Task" || b.ToolName == "Agent" || b.ToolName == "Skill" {
						info := tools.ExtractSubagentInfo(tools.ParseInputFields(b.ToolInput))
						items = append(items, DisplayItem{
							Type:           ItemSubagent,
							ToolName:       b.ToolName,
							ToolID:         b.ToolID,
							ToolInput:      b.ToolInput,
							ToolSummary:    tools.ToolSummary(b.ToolName, b.ToolInput),
							ToolCategory:   tools.CategorizeToolName(b.ToolName),
