...
func TestScanPreview_NormalUserText(t *testing.T) {
	...
	preview, count := discover.ScanSessionPreview(path)
...
func TestScanPreview_CommandFallback(t *testing.T) {
	...
	preview, _ := discover.ScanSessionPreview(path)
	if preview != "/model" {
...
func TestScanPreview_TeammateMessageNotFiltered(t *testing.T) {
	...
	preview, _ := discover.ScanSessionPreview(path)
	// transcript.SanitizeContent strips noise tags but not teammate-message tags.
...
func TestDiscoverProjectSessions_FiltersGhosts(t *testing.T) {
	...
	sessions, err := discover.DiscoverProjectSessions(dir)
	if err != nil {
		t.Fatal(err)
	}
	if len(sessions) != 1 {
...
// --- transcript.ReadSessionIncremental tests ---
...
func TestReadSessionIncremental_IncrementalRead(t *testing.T) {
	...
	// First read.
	msgs1, offset1, err := transcript.ReadSessionIncremental(path, 0)
	if err != nil {
		t.Fatal(err)
	}
	if len(msgs1) != 1 {
	...
	msgs2, offset2, err := transcript.ReadSessionIncremental(path, offset1)
	if err != nil {
		t.Fatal(err)
	}
	if len(msgs2) != 1 {
...
// bytes are consumed. Guards transcript.ReadSession/loadSession on valid JSONL files
...
func TestReadSessionIncremental_CompleteFinalLineNoNewline(t *testing.T) {
	...
	chunks, err := transcript.ReadSession(single)
	if err != nil {
		t.Fatal(err)
	}
	if len(chunks) != 1 {
		t.Fatalf("transcript.ReadSession one-line file: len(chunks) = %d, want 1", len(chunks))
	}
}
...
func TestDiscoverAllProjectSessions_ExactMatchOnly(t *testing.T) {
	...
	sessions, err := discover.DiscoverAllProjectSessions([]string{parentDir})
	if err != nil {
		t.Fatal(err)
	}
	if len(sessions) != 1 {
...
func TestDiscoverAllProjectSessions_MergesAcrossDirs(t *testing.T) {
	...
	sessions, err := discover.DiscoverAllProjectSessions([]string{dir1, dir2})
	if err != nil {
		t.Fatal(err)
	}
	if len(sessions) != 2 {
...
func TestDiscoverAllProjectSessions_SkipsMissingDirs(t *testing.T) {
	...
	sessions, err := discover.DiscoverAllProjectSessions([]string{dir, "/nonexistent/dir"})
	if err != nil {
		t.Fatal(err)
	}
	if len(sessions) != 1 {
...
func TestReadSessionIncremental_NoNewContent(t *testing.T) {
	...
	// Read everything.
	_, offset1, _ := transcript.ReadSessionIncremental(path, 0)

	// Read again from the end -- nothing new.
	msgs, offset2, err := transcript.ReadSessionIncremental(path, offset1)
	if err != nil {
		t.Fatal(err)
	}
	if len(msgs) != 0 {
