...
func TestReadSession_ValidFile(t *testing.T) {
	...
	chunks, err := transcript.ReadSession(path)
	if err != nil {
		t.Fatalf("transcript.ReadSession(%q) error: %v", path, err)
	}
	...
	if chunks[0].Type != transcript.UserChunk {
		t.Errorf("chunks[0].Type = %d, want transcript.UserChunk", chunks[0].Type)
	}
	if chunks[1].Type != transcript.AIChunk {
		t.Errorf("chunks[1].Type = %d, want transcript.AIChunk", chunks[1].Type)
	}
	if chunks[2].Type != transcript.SystemChunk {
		t.Errorf("chunks[2].Type = %d, want transcript.SystemChunk", chunks[2].Type)
	}
}
...
func TestReadSession_EmptyLines(t *testing.T) {
	...
	chunks, err := transcript.ReadSession(path)
	if err != nil {
		t.Fatalf("transcript.ReadSession error: %v", err)
	}
...
func TestReadSession_NoiseFiltered(t *testing.T) {
	...
	// - u1 (user) -> transcript.UserChunk
	// - n1 (system type) -> filtered
	// - n2 (summary type) -> transcript.CompactChunk
	// - a1 (assistant with thinking/tools) -> starts AI buffer
	// - sc1 (sidechain) -> filtered
	// - n3 (synthetic) -> filtered
	// - m1 (meta user) -> transcript.AIMsg (merges into AI buffer with a1)
	// - n4 (system-reminder wrapped) -> filtered
	// - n5 (empty stdout) -> filtered
	// - n6 (interruption) -> filtered
	// - u2 (user) -> flushes AI buffer -> transcript.UserChunk
	// Result: transcript.UserChunk, transcript.CompactChunk, transcript.AIChunk, transcript.UserChunk = 4 chunks
	if len(chunks) != 4 {
	...
	if chunks[1].Type != transcript.CompactChunk {
		t.Errorf("chunks[1].Type = %d, want transcript.CompactChunk", chunks[1].Type)
	}
	if chunks[2].Type != transcript.AIChunk {
		t.Errorf("chunks[2].Type = %d, want transcript.AIChunk", chunks[2].Type)
	}
	if chunks[3].Type != transcript.UserChunk {
		t.Errorf("chunks[3].Type = %d, want transcript.UserChunk", chunks[3].Type)
	}
...
func TestFindTitleMatches(t *testing.T) {
	...
	t.Run("exact match is case-insensitive and unique", func(t *testing.T) {
		got, err := discover.FindTitleMatches("WORKLOG-CRON-CONFIG", dirs)
		if err != nil {
			t.Fatal(err)
		}
		if len(got) != 2 {
	...
	t.Run("substring match wins when no exact", func(t *testing.T) {
		got, err := discover.FindTitleMatches("plugin", dirs)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("ai-title is searchable", func(t *testing.T) {
		got, err := discover.FindTitleMatches("auto-named-thing", dirs)
		if err != nil {
			t.Fatal(err)
		}
		if len(got) != 1 {
	...
	t.Run("exact match preferred over substring", func(t *testing.T) {
		...
		got, err := discover.FindTitleMatches("plug", dirs)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("no match returns empty", func(t *testing.T) {
		got, err := discover.FindTitleMatches("no-such-thing", dirs)
		if err != nil {
			t.Fatal(err)
		}
		if len(got) != 0 {
	...
	t.Run("scanner skips large content lines", func(t *testing.T) {
		...
		got, err := discover.FindTitleMatches("late-rename", []string{heavy})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("empty query returns nil", func(t *testing.T) {
		got, err := discover.FindTitleMatches("  ", dirs)
		if err != nil {
			t.Fatal(err)
		}
		if got != nil {
