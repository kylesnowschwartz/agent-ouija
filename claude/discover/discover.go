...
// Subagent files (see agents.IsAgentSessionFile) are excluded.
...
func listSessionFiles(projectDir string) ([]sessionFile, error) {
	...
	for _, de := range entries {
		...
		if agents.IsAgentSessionFile(name) {
			continue
		}
		info, err := de.Info()
		if err != nil {
...
func discoverSessions(projectDir string, scan scanFn) ([]SessionInfo, error) {
	...
	for _, sf := range files {
		...
		if isOngoing && time.Since(sf.modTime) > transcript.OngoingStalenessThreshold {
			isOngoing = false
		}
		...
		if !isOngoing && agents.ScanWorkflowActivity(sf.path).Active(transcript.OngoingStalenessThreshold) {
			isOngoing = true
		}
...
func scanSessionMetadata(path string) sessionMetadata {
	...
	for {
		...
		if ts := transcript.ParseTimestamp(raw.Timestamp); !ts.IsZero() {
			if firstTS.IsZero() {
				firstTS = ts
			}
			lastTS = ts
		...
		if raw.Type == "assistant" && !raw.IsSidechain && raw.Message.Model != "<synthetic>" {
			...
			meta.contextTokens = transcript.Usage{
		...
		text := transcript.ExtractText(raw.Message.Content)
		if text == "" {
			continue
		}

		if transcript.IsCommandOutput(text) || strings.HasPrefix(text, "[Request interrupted by user") {
			continue
		}

		if strings.HasPrefix(text, "<command-name>") {
			if commandFallback == "" {
				if m := pat.CommandNameRe.FindStringSubmatch(text); m != nil {
		...
		sanitized := strings.TrimSpace(transcript.SanitizeContent(text))
		if sanitized == "" {
			continue
		}
...
type metadataScanEntry struct {
	...
	Message        struct {
		...
		Usage   transcript.EntryUsage `json:"usage"`
	} `json:"message"`
}
...
func isUserChunkForTurnCount(e *metadataScanEntry) bool {
	...
	text := transcript.ExtractText(e.Message.Content)
	trimmed := strings.TrimSpace(text)

	// Teammate messages.
	if pat.TeammateMessageRe.MatchString(trimmed) {
		return false
	}

	// System output tags.
	for _, tag := range pat.SystemOutputTags {
	...
	return transcript.HasUserContent(e.Message.Content, text)
}
	...
	for _, b := range blocks {
		...
		case "tool_use":
			...
			if b.Name == "ExitPlanMode" {
				...
				*hasAfter = false
				*activityIndex++
			} else if transcript.IsShutdownApproval(b.Name, b.Input) {
