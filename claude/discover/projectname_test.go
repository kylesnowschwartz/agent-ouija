...
func TestFindGitRepoRoot(t *testing.T) {
	t.Run("normal repo from subdir", func(t *testing.T) {
		...
		got := gitroot.FindRepoRoot(sub)
		if got != repo {
			t.Fatalf("gitroot.FindRepoRoot(%q) = %q, want %q", sub, got, repo)
		}
	})
	...
	t.Run("worktree with commondir", func(t *testing.T) {
		...
		got := gitroot.FindRepoRoot(wt)
		if got != mainRepo {
			t.Fatalf("gitroot.FindRepoRoot(%q) = %q, want %q", wt, got, mainRepo)
		}
	})
	...
	t.Run("no .git returns empty", func(t *testing.T) {
		...
		got := gitroot.FindRepoRoot(dir)
		if got != "" {
			t.Fatalf("gitroot.FindRepoRoot(%q) = %q, want %q", dir, got, "")
		}
	})

	t.Run("empty string returns empty", func(t *testing.T) {
		got := gitroot.FindRepoRoot("")
		if got != "" {
			t.Fatalf("gitroot.FindRepoRoot(%q) = %q, want %q", "", got, "")
		}
	})
}
