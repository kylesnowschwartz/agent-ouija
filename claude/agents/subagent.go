...
type SubagentProcess struct {
	...
	Chunks        []transcript.Chunk // parsed via transcript.ReadSession pipeline
	StartTime     time.Time          // first message timestamp
	EndTime       time.Time          // last message timestamp
	DurationMs    int64
	Usage         transcript.Usage // last AI chunk's context-window snapshot (not a sum)
...
func chunkTiming(chunks []transcript.Chunk) (start, end time.Time, durationMs int64) {
...
// content before transcript.Classify strips the XML tag attributes.
//
...
//
// Unlike transcript.ReadSession, it ignores the isSidechain flag since all entries
// in subagent files are marked isSidechain=true but represent the
// subagent's own main conversation.
func readSubagentSession(path, trustedRoot string) ([]transcript.Chunk, string, string, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, "", "", err
	}
	...
	for {
		...
		// <teammate-message> tag before transcript.Classify strips the XML attributes.
		if !extractedTeamMeta && entry.Type == "user" {
			var contentStr string
			if json.Unmarshal(entry.Message.Content, &contentStr) == nil {
				if m := pat.TeammateSummaryRe.FindStringSubmatch(contentStr); len(m) > 1 {
					teamSummary = m[1]
				}
				if m := pat.TeammateColorRe.FindStringSubmatch(contentStr); len(m) > 1 {
					teamColor = m[1]
				}
		...
		// Clear sidechain flag so transcript.Classify doesn't filter these out.
		entry.IsSidechain = false
		msg, ok := transcript.Classify(entry)
		if !ok {
			continue
		}
	...
	transcript.ResolvePersistedOutputs(msgs, trustedRoot)

	return transcript.BuildChunks(msgs), teamSummary, teamColor, nil
}

// extractModel returns the model string from the first AI chunk, or "".
func extractModel(chunks []transcript.Chunk) string {
	for _, c := range chunks {
...
func lastUsageSnapshot(chunks []transcript.Chunk) transcript.Usage {
	for i := len(chunks) - 1; i >= 0; i-- {
		if chunks[i].Type == transcript.AIChunk && chunks[i].Usage.TotalTokens() > 0 {
...
func LinkSubagents(processes []SubagentProcess, parentChunks []transcript.Chunk, parentSessionPath string) map[string]string {
	...
	var taskItems []*transcript.DisplayItem
	...
	// Build tool_use_id -> transcript.DisplayItem for enrichment.
	toolIDToTask := make(map[string]*transcript.DisplayItem, len(taskItems))
	...
	var unmatchedTasks []*transcript.DisplayItem
	for _, it := range taskItems {
...
func filterTeamTasks(items []*transcript.DisplayItem, matched map[string]bool) []*transcript.DisplayItem {
	var out []*transcript.DisplayItem
...
// IsTeamTask checks whether a Task transcript.DisplayItem's input contains both
// team_name and name keys, marking it as a team member spawn.
func IsTeamTask(it *transcript.DisplayItem) bool {
...
func teamSpecFromInput(input json.RawMessage) (teamName, agentName string, ok bool) {
	...
	return tools.GetString(fields, "team_name"), tools.GetString(fields, "name"), true
}
...
func scanAgentLinks(sessionPath string) agentLinkData {
	...
	for {
		...
		agentID := tools.GetString(resultMap, "agentId")
		if agentID == "" {
			agentID = tools.GetString(resultMap, "agent_id")
		}
		if agentID == "" {
			continue
		}
		...
		if color := tools.GetString(resultMap, "color"); color != "" {
...
func extractFirstToolResultID(entry transcript.Entry) string {
	var blocks []struct {
		Type      string `json:"type"`
...
func enrichProcess(proc *SubagentProcess, item *transcript.DisplayItem) {
...
func extractTeamSpecs(chunks []transcript.Chunk) []teamSpec {
	var specs []teamSpec
	for i := range chunks {
...
func DiscoverTeamSessions(sessionPath string, parentChunks []transcript.Chunk) ([]SubagentProcess, error) {
