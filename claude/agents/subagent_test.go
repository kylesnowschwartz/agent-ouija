...
func TestDiscoverSubagents_FindsValidAgents(t *testing.T) {
	...
	procs, err := agents.DiscoverSubagents(sessionPath)
	if err != nil {
...
func TestDiscoverSubagents_ParsesChunks(t *testing.T) {
	...
	// But subagent entries have isSidechain=true, which transcript.Classify filters.
...
func TestIsAgentSessionFile(t *testing.T) {
	...
	for _, c := range cases {
		if got := agents.IsAgentSessionFile(c.name); got != c.want {
...
// makeTaskChunk builds an AI chunk with a Task tool call transcript.DisplayItem.
func makeTaskChunk(toolID, subagentType, desc string) transcript.Chunk {
...
func makeTeamTaskChunk(toolID, subagentType, desc, teamName, memberName string) transcript.Chunk {
	input, _ := json.Marshal(map[string]string{
		"subagent_type": subagentType,
		"description":   desc,
		"team_name":     teamName,
...
// makeTeamSubagentWithSummary builds a SubagentProcess whose first transcript.UserChunk
...
func makeTeamSubagentWithSummary(id string, startTime time.Time, summary string) agents.SubagentProcess {
	return agents.SubagentProcess{
		ID:          id,
		StartTime:   startTime,
		TeamSummary: summary,
		Chunks: []transcript.Chunk{
			{Type: transcript.AIChunk, Timestamp: startTime, Model: "claude-haiku-4-5"},
			{Type: transcript.AIChunk, Timestamp: startTime.Add(time.Second), Model: "claude-haiku-4-5"},
		},
	}
}

func TestLinkSubagents_ResultBased(t *testing.T) {
	procs := []agents.SubagentProcess{
	...
	agents.LinkSubagents(procs, chunks, parentPath)
...
func TestLinkSubagents_UnmatchedKeepsEmpty(t *testing.T) {
	...
	agents.LinkSubagents(procs, nil, "")
...
func TestLinkSubagents_NoProcesses(t *testing.T) {
	...
	agents.LinkSubagents(nil, chunks, "")
}
	...
	procs := []agents.SubagentProcess{
		{
			...
			Chunks: []transcript.Chunk{
				{Type: transcript.UserChunk, UserText: "Just a regular user message"},
			},
		},
	}
	chunks := []transcript.Chunk{
	...
	procs := []agents.SubagentProcess{
		{
			...
			Chunks: []transcript.Chunk{
				{Type: transcript.AIChunk, Model: "claude-haiku-4-5"},
			},
		},
	}
	chunks := []transcript.Chunk{
...
func TestTeamLinkingIntegration(t *testing.T) {
	...
	// DiscoverSubagents -> transcript.ReadSession -> transcript.BuildChunks -> LinkSubagents.
	//
	...
	// ExtractTeamMessageSummary operated on chunk text (post-transcript.Classify)
	// instead of raw entry content (pre-transcript.Classify).
	parentPath := filepath.Join("../testdata", "team-parent.jsonl")

	// Step 1: Discover subagents — exercises readSubagentSession which
	// extracts TeamSummary from raw entry content before transcript.Classify strips it.
	procs, err := agents.DiscoverSubagents(parentPath)
	if err != nil {
	...
	parentChunks, err := transcript.ReadSession(parentPath)
	if err != nil {
		t.Fatalf("transcript.ReadSession: %v", err)
	}

	// Parent should have: 1 transcript.UserChunk + 3 AIChunks (each with a Task tool_use + tool_result).
	...
	agents.LinkSubagents(procs, parentChunks, parentPath)

	// Build lookup for assertions.
	procByID := make(map[string]agents.SubagentProcess, len(procs))
	for _, p := range procs {
		procByID[p.ID] = p
	}
...
func TestReadTeamSessionMeta_TeamSession(t *testing.T) {
	...
	teamName, agentName := agents.ReadTeamSessionMeta(path)
	if teamName != "my-team" {
...
func TestReadTeamSessionMeta_MissingFile(t *testing.T) {
	teamName, agentName := agents.ReadTeamSessionMeta("/nonexistent/path.jsonl")
	if teamName != "" || agentName != "" {
...
func TestDiscoverTeamSessions_FindsTeamFiles(t *testing.T) {
	...
	classified, _, err := transcript.ReadSessionIncremental(parentPath, 0)
	if err != nil {
		t.Fatalf("transcript.ReadSessionIncremental: %v", err)
	}
	chunks := transcript.BuildChunks(classified)

	procs, err := agents.DiscoverTeamSessions(parentPath, chunks)
	if err != nil {
...
func TestLinkSubagents_TeamSessionPhase1(t *testing.T) {
	...
	procs := []agents.SubagentProcess{
		{
			...
			Chunks:    []transcript.Chunk{{Type: transcript.AIChunk, Model: "claude-sonnet-4-20250514"}},
		},
		{
	...
	colorMap := agents.LinkSubagents(procs, chunks, parentPath)
...
func TestTeamSessionDiscoveryAndLinking(t *testing.T) {
	...
	subagents, _ := agents.DiscoverSubagents(parentPath)
	teamProcs, err := agents.DiscoverTeamSessions(parentPath, chunks)
	if err != nil {
		t.Fatalf("DiscoverTeamSessions: %v", err)
	}

	allProcs := append(subagents, teamProcs...)
	colorMap := agents.LinkSubagents(allProcs, chunks, parentPath)

	// Team sessions should be linked via Phase 1 (name@team -> tool_use_id).
	procByID := make(map[string]agents.SubagentProcess)
	...
	for _, p := range allProcs {
		...
		for _, c := range p.Chunks {
			if c.Type == transcript.AIChunk && c.Model != "" {
				hasAI = true
				break
			}
		}
...
func TestTeamSpecFromInput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			team, agent, ok := agents.TeamSpecFromInput(json.RawMessage(tt.input))
